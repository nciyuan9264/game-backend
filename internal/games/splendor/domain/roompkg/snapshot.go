...
func (r *RoomService) buildSnapshot() RoomSnapshot {
	...
	if room.State != nil {
		...
		snapshot.MaxPlayers = room.State.MaxPlayers
		snapshot.MaxScore = maxPlayerScore(room.State.Players)
	}
...
func maxPlayerScore(players map[string]*domain.PlayerState) int {
	maxScore := 0
	for _, player := range players {
		if player != nil && player.Score > maxScore {
